...
class HasarPrinter(PrinterInterface):
		...
		CMD_ADD_PAYMENT = 0x44 #TotalTender de 320F
		...
		#Notice, if at some point, the 441 behaves diferently than 320, we'll need to create a new model for it.
		#TODO change this models to numbers. maybe delete the whole array
		AVAILABLE_MODELS = ("615", "715v1", "715v2", MODEL_320)
		ivaTypeMap = {
		...
		textSizeDict = {#Esto podria ser una clase diferente
		...
		DAILY_CLOSE_X = 'X'
		DAILY_CLOSE_Z = 'Z'
		def __init__(self, deviceFile=None, speed=9600, host=None, port=None, 
			model="615", dummy=False, connectOnEveryCommand=False):
			try:
				if dummy:
		...
		def _setHeaderTrailer(self, line, text):
			#if self.model == self.MODEL_320: return
		...
		def setTrailer(self, trailer=None):
			"""Establecer pie"""
			#Esto es lo mismo que el header, pero cambia la linea, y segun la documentacion en escencia es lo mismo solo
			#cambia la numeracion, seria mejor tenerlo todo en el mismo codigo, pej, _setHeaderTRailer
			if not trailer:
				trailer = []
			fill = '\x7F'
			if self.model == self.MODEL_320: fill = '-'
			lines = len(trailer)
...
##  NO SE PUEDE
		...
		def addItem(self, description, quantity, price, iva, discount=0.0, discountDescription=None, negative=False):
				...
				#Todo create function for adding description
				...
				if discount:
					#Model 320 cant add a discount of 0.0
		...
		def openDrawer(self):
				if not self.model in (self.MODEL_320, "615"):
